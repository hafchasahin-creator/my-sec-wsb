...
Two Minecraft **Bedrock Edition** add-ons live in this repository, both built and
verified against the format versions available in **Bedrock 1.21.0** (the Beta
`1.21.0.26` build, Android / Pocket Edition):

1. **Magical Swords** (new) - five animated magical swords: `dist/MagicalSwords.mcaddon`
2. **Arcane Arsenal** - six legendary weapons: `dist/ArcaneArsenal.mcaddon`

Both use only **stable** item components and the **stable** `@minecraft/server 1.11.0`
scripting module, so **no experimental toggles are required**.

---

# Magical Swords

Five swords, each with its own 3D animated blade model, emissive energy layer,
particle aura, custom synthesized sounds and a touch-friendly special ability.
Everything was validated against Mojang's published `bedrock-samples v1.21.0.3`
data (vanilla resource pack, item component JSON schemas and the
`@minecraft/server 1.11.0` module metadata), so nothing newer than 1.21.0 is used.

## The swords

All five appear in the Creative inventory under **Equipment -> swords**, are craftable,
enchantable and repairable. While one is held, a **special-ability button appears on
touch screens** (the `minecraft:interact_button` component - no `/function` commands,
no experiments). On keyboard/mouse the same ability is on right-click.

| Sword | On hit | Use button | Sneak + Use |
| --- | --- | --- | --- |
| **§6Inferno Sword§r** (9 dmg) | ignites enemies (6 s) + ember burst | **Flame Cyclone** - expanding ring of fire, 7 dmg + ignite + knockback (8 s cooldown) | - |
| **§bFrost Sword§r** (8 dmg) | Slowness III, +2 freeze dmg, icy shatter; frozen enemies visibly frost over | **Glacial Wave** - travelling widening ice wave, 6 dmg + Slowness IV (8 s) | - |
| **§eThunder Sword§r** (9 dmg) | 25 % chance to summon lightning; electric crackle | **Skyfall Bolt** - lightning strike at the aimed spot (40 blocks) + shock ring, 8 dmg (6 s) | - |
| **§5Void Sword§r** (10 dmg) | +2 void dmg, dark wisps | **Void Rift** - travelling void slash, 12 dmg (5 s) | **Blink** - teleport up to 8 blocks forward (wall-safe) |
| **§dCelestial Sword§r** (10 dmg) | +2 astral dmg, star chimes | **Astral Rend** - huge travelling energy slash, 14 dmg + knock-up (10 s) | **Starfall Nova** - celestial explosion at the aimed target (30 blocks), 18 dmg |

Ability cooldowns show as the ender-pearl style radial sweep on the hotbar icon, and
the action bar reports the remaining time if you press early. Equipping a sword shows
a one-line control hint in the action bar.

## Crafting (crafting table, unlocked from the start)

| Sword | Recipe (top -> bottom, centre column) |
| --- | --- |
| Inferno | Fire Charge / Magma Block / Blaze Rod |
| Frost | Blue Ice / Diamond / Stick |
| Thunder | Lightning Rod / Diamond / Stick |
| Void | Eye of Ender / Crying Obsidian / Blaze Rod |
| Celestial | Amethyst + **Nether Star** + Amethyst on top, then Diamond Block, Blaze Rod |

Or grab them instantly:

```
/give @s msword:inferno_sword
/give @s msword:frost_sword
/give @s msword:thunder_sword
/give @s msword:void_sword
/give @s msword:celestial_sword
```

## How the visuals work (all 1.21.0-safe formats)

- **3D blades**: each sword is a resource-pack *attachable* bound to the hand exactly
  the way the vanilla trident is (`geometry` format 1.16.0, bone
  `binding: q.item_slot_to_bone_name(c.item_slot)`), with separate first-person and
  third-person hold poses driven by an animation controller on `c.is_first_person`.
- **Animated energy**: every blade has an emissive core (`entity_emissive` material)
  on its own render controller that UV-scrolls a tileable energy sheet
  (`uv_anim`, the same mechanism the vanilla Breeze uses) and pulses/floats via
  looping Molang animations. Void and Celestial also have orbiting shard/star cubes.
- **Held particle auras**: the looping idle animation fires the sword's aura particles
  from geometry locators (blade tip and mid-blade) - flames flow around the Inferno
  blade, snow drifts off the Frost blade, sparks jump on the Thunder blade, wisps
  orbit the Void blade and stars circle the Celestial blade. A light scripted aura
  backs this up so it reads in first person too.
- **17 custom particle systems** (`particles/*.json`, format 1.10.0): flipbook flames,
  orbiting wisps (parametric motion), ice shatters, spark bursts, rift arcs, the
  frozen-enemy shell and the celestial nova shell.
- **9 synthesized OGG sounds** (`sound_definitions.json`): every ability has a bespoke
  cast sound plus an equip shimmer, generated by `tools/gen_ms_sounds.py`.

If a hold pose looks slightly off on some device, the two tuning knobs are
`animation.msword.<sword>.first_person_hold` and `...third_person_hold` in
`resource_packs/magical_swords_rp/animations/`.

## Installing on Android

1. Copy **`dist/MagicalSwords.mcaddon`** to the device and tap it - Minecraft imports
   both packs.
2. World -> **Behavior Packs** -> activate **Magical Swords BP** (the resource pack is
   pulled in automatically via the manifest dependency).
3. Leave every experimental toggle **off** - none are needed.

On spawn, chat prints `[Magical Swords] v1.0.0 loaded - 5 blades in the Creative
equipment tab.` If that line is missing the scripts are not running; check
**Settings -> Creator -> Content Log GUI** for the reason.

## Rebuilding Magical Swords

```bash
python3 tools/gen_ms_rp.py         # regenerate models/attachables/animations/particles
python3 tools/gen_ms_textures.py   # regenerate all textures + icons (needs Pillow)
python3 tools/gen_ms_sounds.py     # resynthesize sounds (needs numpy + soundfile)
python3 tools/build_ms.py          # validate every cross-reference + write the .mcaddon
```

`build_ms.py` walks every reference chain (item -> icon -> png, attachable ->
geometry/animations/particles/textures, script -> particle & sound ids, recipes ->
vanilla item ids) and refuses to package if anything dangles.
