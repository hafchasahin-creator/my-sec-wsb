...
const CONFIG = {
    ...
    playlist: [ ... ],          // generated by the build from assets/
...
- `profilePicture`, `backgroundImage`, `playlist[].file` and `playlist[].cover`
  ...
  and rebuild.
- **Album art:** the supplied MP3s have no embedded cover, so
  `assets/cover-placeholder.jpg` (a crop of the background) is used for every
  track. Drop a real cover into `assets/` and point the build at it to change
  that.
- **The playlist builds itself.** `song.mp3` leads, then every `track-*.mp3` in
  `assets/` in name order. To add a song, drop it in as `track-4.mp3` and
  rebuild — `CONFIG.playlist` is generated, so there is nothing to edit. Each
  track is titled from its own ID3 tag (`TIT2` / `TPE1`), falling back to the
  filename when a file carries no tag. Tag an untagged file with:

  ```sh
  ffmpeg -i in.mp3 -c:a copy -metadata title="Name" -metadata artist="Who" \
         assets/track-4.mp3
  ```

- Previous / next cycle through the playlist and wrap at both ends, and a track
  that finishes advances to the next one. With a single track those buttons
  restart it instead.
